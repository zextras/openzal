...
public interface ZEStoreManager
{
  ...
  ZEMailboxBlob copy(ZEBlob src, ZEMailbox destMbox, int destMsgId, int destRevision, short destVolumeId)
    throws IOException, ZimbraException;

  ZEMailboxBlob link(ZEBlob src, ZEMailbox destMbox, int destMsgId, int destRevision, short destVolumeId)
    throws IOException, ZimbraException;

  String getBlobPath(int mboxId, int itemId, int revision,
                     short volumeId) throws org.openzal.zal.exceptions.ZimbraException;

  boolean delete(ZEBlob blob) throws IOException;

  boolean delete(ZEMailboxBlob mblob) throws IOException;
}
